...
pub enum Instruction {
    ...
    STMR { x: u8, y: u8 },
    ADDI { x: u8, address: u16 },
    ADDR2 { x: u8, y: u8 },
    ADDR3 { x: u8, y: u8, z: u8 },
    SUBI { x: u8, address: u16 },
    SUBR2 { x: u8, y: u8 },
    SUBR3 { x: u8, y: u8, z: u8 },
    CMPI { x: u8, address: u16 },
    CMPR { x: u8, y: u8 },
    ANDI { x: u8, address: u16 },
    ANDR2 { x: u8, y: u8 },
    ANDR3 { x: u8, y: u8, z: u8 },
    TSTI { x: u8, address: u16 },
    TSTR { x: u8, y: u8 },
    ORI { x: u8, address: u16 },
    ORR2 { x: u8, y: u8 },
    ORR3 { x: u8, y: u8, z: u8 },
    XORI { x: u8, address: u16 },
    XORR2 { x: u8, y: u8 },
    XORR3 { x: u8, y: u8, z: u8 },
    MULI { x: u8, address: u16 },
    MULR2 { x: u8, y: u8 },
    MULR3 { x: u8, y: u8, z: u8 },
    DIVI { x: u8, address: u16 },
    DIVR2 { x: u8, y: u8 },
    DIVR3 { x: u8, y: u8, z: u8 },
    MODI { x: u8, address: u16 },
    MODR2 { x: u8, y: u8 },
    MODR3 { x: u8, y: u8, z: u8 },
    REMI { x: u8, address: u16 },
    REMR2 { x: u8, y: u8 },
    ...
    PALI { address: u16 },
    PALR { x: u8 },
    NOTI { x: u8, address: u16 },
    NOTR1 { x: u8 },
    NOTR2 { x: u8, y: u8 },
    NEGI { x: u8, address: u16 },
    NEGR1 { x: u8 },
    NEGR2 { x: u8, y: u8 },
}
...
impl Instruction {
    pub fn decode(opcode: u32) -> Result<Instruction, &'static str> {
        ...
        match opcode.ii() {
            ...
            0x20 => Ok(LDIR { x: opcode.x(), address: opcode.hhll() }),
            0x21 => Ok(LDIS { address: opcode.hhll() }),
            ...
            0xE0 => Ok(NOTI { x: opcode.x(), address: opcode.hhll() }),
            0xE1 => Ok(NOTR1 { x: opcode.x() }),
            0xE2 => Ok(NOTR2 { x: opcode.x(), y: opcode.y() }),
            0xE3 => Ok(NEGI { x: opcode.x(), address: opcode.hhll() }),
    ...
    pub fn disassemble(&self) -> String {
        ...
        match self {
            ...
            DRWI { x, y, address } => format!("DRWI"),
            DRWR { x, y, z } => format!("DRWR"),
            RND { x, address } => format!("RND"),
            FLIP { flip_horizontal, flip_vertical } => format!("FLIP"),
            SND0 => format!("SND0"),
            SND1 { address } => format!("SND1"),
            SND2 { address } => format!("SND2"),
            SND3 { address } => format!("SND3"),
            ...
            PALR { x } => format!("PALR"),
            NOTI { x, address } => format!("NOTI"),
            NOTR1 { x } => format!("NOTR1"),
            NOTR2 { x, y } => format!("NOTR2"),
            NEGI { x, address } => format!("NEGI"),
